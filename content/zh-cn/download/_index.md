...
- **仓库**：<https://github.com/Gig-OS/Live-ISO>（包含构建脚本与相关定制）
...
**需要在虚拟机里运行？** 镜像按 `x86-64-v3` 编译，必须要有 AVX2。而 **VirtualBox 通常无法透传 AVX2，因此镜像无法启动**——建议换用 **KVM（`-cpu host`）、原生 Hyper-V 或 VMware**；具体是否支持 AVX2 以 guest 里 `grep -o avx2 /proc/cpuinfo` 为准。
{{< /callout >}}

{{% details title="这个 Live ISO 有什么（点开看更多）" %}}

- **三语言支持** — GRUB 菜单可以选择 简体 / 正體 / English，桌面语言、Firefox 语言、相关的输入法将会跟随切换。
- **多种启动方式** — 除常规启动外，还有加载到内存运行启动项（将整盘载入内存后运行，可以移除 U 盘，运行速度更快），以及安全显卡模式启动项。
- **中文输入法 fcitx5 + rime** — 默认朙月拼音；**右键托盘输入法图标，选择方案** 可切换 注音 / 五笔86 / 仓颉 / 粤拼 等。
- **开源 / 闭源显卡** — 默认 nouveau 即插即用；新卡（RTX 20/30/40/50）想要硬件加速请选择闭源 NVIDIA 启动项，**需提前在 UEFI 中关闭 Secure Boot**（因为驱动未签名，无法加载）。如果仍然无法正常显示，请使用安全显卡模式启动项。
- **图形安装器（可选）** — 桌面双击安装系统图标可启动 Calamares 图形安装器（跟随所选语言），安装完成后将会自动清理 live 残留（开机自动登录等）；希望手动安装、深入了解系统，推荐按照官方手册逐步操作（见下方说明）。
- **ZFS 根 + 原生加密 + ZBM 引导（进阶）** — 安装器分区页文件系统可选 **ZFS**；勾选加密选项即启用 **ZFS 原生加密**（aes-256-gcm、口令解锁），并由 **ZFSBootMenu** 原生引导（因为 GRUB 无法读取带新特性 / 原生加密的 ZFS 池，所以 ZFS 根改用 ZBM）。默认文件系统为 btrfs，在分区页面可选择 xfs / ext4 / ZFS 文件系统。
- **CPU_FLAGS_X86** — 装好系统后，这项编译参数将按你的 CPU 自动生成。

完整功能与配置说明见 **[镜像站的使用说明页](https://iso.gentoozh.org/about.html)**。
