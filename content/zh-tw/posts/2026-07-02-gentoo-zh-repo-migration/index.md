...
在 `metadata/news/` 中新增一個符合 GLEP 42 的 repository news item，標題為 gentoo-zh overlay moved to gentoo-zh/overlay，同時提供 `.en.txt` 和 `.zh.txt` 兩個檔案。使用者同步 overlay 後可透過 `eselect news read` 讀取。
