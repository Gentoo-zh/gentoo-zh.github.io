...
監控也從單機撥測升級成**兩套異地 Prometheus/Alertmanager 互相盯著、叢集去重**——任一套（連同它所在的機房）掛了，另一套仍會發告警，不會出現監控和服務一起靜默的情況。主機、四個網站的可用性、SSL 到期都在盯，告警即時進 Telegram，狀態同步公開到 [status.gentoozh.org](https://status.gentoozh.org)。
